...
Fetch via `pi-web-access` (URL / DOI / PDF). Write **one** note under `Papers & Reviews/` with callouts: `> [!abstract]` claim, `> [!info]` method, `> [!example]` evidence, `> [!warning]` limitations, plus `[[wikilinks]]` to existing AL/Programme notes. If the source is a URL, include a ` ```cardlink ` fence. `/preview` if it has math.
