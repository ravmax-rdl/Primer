...
- Worked solutions in adjacent text nodes, graded from lecture notes, labelled unofficial for BSc.
- Model writes a `graph.json`; `layout.py` assigns coordinates. Merge if a canvas already exists. Write a `.canvas` next to the paper or under `Papers & Reviews/BSc/`.
